...
with torch.no_grad():
    # New model returns 4 values
    corners, conf, p_sz, p_R = model(batch['pc'], batch['obj_pc'], batch['mask'], batch['rgb'])

print("\n"+"="*60)
print("DIRECT SUPERVISION TEST")
print("="*60)
total, ctr, sz, ori, cnf = hybrid_3d_loss(corners, conf, batch['bbox'], batch['valid'],
                                         pred_s=p_sz, pred_R=p_R)
...
_, _, v_sz, v_ori, _ = hybrid_3d_loss(corners[:1,:1], conf[:1,:1], empty_bbox, empty_mask)
print(f"Empty Slot Size Loss (should NOT be NaN): {v_sz.item()}")
print(f"Empty Slot Orient Loss (should NOT be NaN): {v_ori.item()}")

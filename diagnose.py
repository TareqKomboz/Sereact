...
with torch.no_grad():
    ...
    p_c, p_s, p_R, p_conf = model(batch['pc'], batch['obj_pc'], batch['mask'], batch['rgb'])

print("\n"+"="*60)
print("DIRECT SUPERVISION TEST")
print("="*60)
# hybrid_3d_loss(pred_c, pred_s, pred_R, pred_conf, true_corners, valid_mask)
total, ctr, sz, ori, cnf = hybrid_3d_loss(p_c, p_s, p_R, p_conf, batch['bbox'], batch['valid'])
...
_, ctr_v, sz_v, ori_v, _ = hybrid_3d_loss(p_c[:1,:1], p_s[:1,:1], p_R[:1,:1], p_conf[:1,:1], empty_bbox, empty_mask)

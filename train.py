...
def train_epoch(model, dataloader, optimizer, device, scheduler=None):
    """
    Optimizes multi-component geometric bbox loss.
    """
    model.train()
    ...
    for batch in dataloader:
        ...
        # Model returns bbox components only.
        pred_c, pred_s, pred_orient, pred_s_log = model(pc, obj_pc, obj_indices, rgb, mask)
        
        # Pass model to loss to enable IoU calculation
        losses = hybrid_3d_loss(pred_c, pred_s_log, pred_orient, bbox, valid, model=model)
...
def test_epoch(model, dataloader, device):
    ...
    with torch.no_grad():
        for b in dataloader:
            ...
            p_c, p_s, p_orient, p_s_log = model(pc, obj_pc, obj_indices, rgb, mask)
            losses = hybrid_3d_loss(p_c, p_s_log, p_orient, bbox, valid, model=model)
